...
OUT=~/cap_xrel2; mkdir -p $OUT ~/xrel
...
adb -s $SER shell pm path $PKG >/dev/null 2>&1 || { echo INSTALL_FAILED; echo GXREL2_DONE; exit 0; }
...
$FT/fritap -m $SER -s -k $OUT/keys.log -p $OUT/traffic.pcap "$PKG" -v > ~/xrel2.log 2>&1 &
FP=$!
sleep 30
...
grep -aiE "libssl|conscrypt|SSL_read|SSL_write|keylog|terminated|No TLS" ~/xrel2.log 2>/dev/null | grep -vaE "^[0-9A-F]{8}:" | tail -8
