...
def _align_native_lib_16k(src: Path, dst: Path) -> bool:
    ...
    padding, and grows the file accordingly. The section-header table is kept and
    its e_shoff + every sh_offset are shifted by the same padding (zeroing it out
    trips the linker's "invalid e_shstrndx" check).
