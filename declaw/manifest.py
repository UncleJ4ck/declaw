...
def _find_smali_for_class(unpacked: Path, fq_class: str) -> Optional[Path]:
    ...
    for smali_root in sorted(unpacked.glob("smali*")):
        ...
        # a crafted android:name with a literal '/' could resolve outside smali_root;
        # only accept a candidate that stays within it.
        try:
            candidate.resolve().relative_to(smali_root.resolve())
        except ValueError:
            continue
