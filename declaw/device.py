...
from declaw.config import ADB_HOST, ADB_PORT, DEFAULT_PROXY_HOST, DEFAULT_PROXY_PORT, log, safe_pkg
...
def pull_package(device: AdbDevice, package: str, dest: Path) -> list[Path]:
    package = safe_pkg(package)   # no shell metacharacters into the pm-path command
