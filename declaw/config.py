...
def safe_pkg(pkg: str) -> str:
    """Validate an Android package name (only ``[A-Za-z0-9._]``) so it is safe to
    interpolate into adb-shell commands and filesystem paths. Blocks shell
    metacharacters and path separators (no injection, no ``..``/``/`` traversal).
    Raises ValueError on anything else."""
    # `..` (and `.`/`...`) pass the charclass but are path traversal: `PACKAGES_DIR / '..'`
    # escapes the cache dir. A real package always has a non-dot segment.
    if not pkg or not _PKG_RE.match(pkg) or ".." in pkg or pkg.strip(".") == "":
